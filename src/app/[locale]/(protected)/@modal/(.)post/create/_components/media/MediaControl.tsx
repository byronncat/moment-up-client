...
export default function MediaView({ onBrowse }: MediaViewProps) {
  const { files, setFiles, setPhase } = usePostData();
          ...
          {files.length < MAX_FILES_LIMIT && (
            <div className="p-2 w-full aspect-square">
              <button
                onClick={onBrowse}
                className={cn(
                  "group size-full rounded-lg cursor-pointer",
              ...
              >
                <Plus
                  className={cn(
                    "size-8",
                    "fill-muted-foreground/40 group-hover:fill-accent/50",
                    "group-focus-visible:fill-accent/50",
                    "transition-colors duration-150"
                  )}
                />
                <span
                  className={cn(
                    "text-xs",
                    "text-muted-foreground/40 group-hover:text-accent/50",
                    "group-focus-visible:text-accent/50",
                    "transition-colors duration-150"
                  )}
                >
                  Add more
