...
import { useCloudinary, useKey, useRefreshApi } from "@/components/providers";
...
export default function PostDataProvider({
  ...
  const upload = useCallback(async () => {
    ...
    if (files.length > 0) {
      const { success, data } = await uploadMultipleImages(files, "moment-up");
      if (!success) {
    ...
    setIsUploading(false);
    return false;
  }, [text, privacy, files, uploadApi, uploadMultipleImages, incrementPostKey]);
