...
export default function StoryDataProvider({
  ...
  const allStories = myStory ? [myStory, ...otherStories] : otherStories;
  ...
  function setStories(stories: StoryNotificationInfo[]) {
    const mine =
      stories.find((story) => story.username === user?.username) ?? null;
    const others = stories.filter((story) => story.username !== user?.username);
    setOtherStories(others);
    setMyStory(mine);
  }

  function nextUserStory(direction: "next" | "prev" = "next") {
    ...
    if (direction === "next") {
      ...
      nextIndex =
        currentStoryIndex < otherStories.length - 1 ? currentStoryIndex + 1 : 0;
      ...
      nextIndex =
        currentStoryIndex > 0 ? currentStoryIndex - 1 : otherStories.length - 1;
  ...
  function muteStory(username: string) {
